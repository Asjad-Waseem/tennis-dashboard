...
const LatestScores = (): React.ReactElement => {
  ...
  if (isLoading || !latestMatch) {
    return (
      <div className="relative h-auto min-h-[240px] w-full max-w-full lg:w-96 rounded-2xl sm:rounded-3xl bg-white">
        <div className="h-full w-full animate-pulse rounded-2xl sm:rounded-3xl bg-gray-100" />
      </div>
    );
  }
  ...
  return (
    <div className="relative h-auto min-h-[240px] w-full max-w-full lg:w-96 rounded-2xl sm:rounded-3xl bg-white">
      ...
      <div className="flex items-center justify-between p-4 sm:p-5 pb-3 sm:pb-4">
        <h2 className="text-lg sm:text-xl font-medium leading-5 sm:leading-6 tracking-wide text-gray-900">
          Latest Scores
        </h2>
        <a
          href="#"
          className="cursor-pointer text-xs sm:text-sm lg:text-[15px] font-semibold leading-[14px] sm:leading-[16px] lg:leading-[18px] tracking-wide text-blue-600 hover:opacity-80"
        >
          View All
        </a>
      </div>
      ...
      {/* Tabs Section */}
      <div className="flex h-auto flex-row items-center gap-2 sm:gap-3 md:gap-4 px-4 sm:px-5 pb-3 sm:pb-4">
        {/* Singles Tab */}
        <button
          type="button"
          onClick={() => setActiveTab("Singles")}
          className={`flex h-auto cursor-pointer flex-row items-center whitespace-nowrap rounded-md transition-all ${
            activeTab === "Singles" ? "bg-cyan-50 px-1.5 py-0.5" : "px-0"
          }`}
        >
          <span
            className={`whitespace-nowrap text-xs sm:text-sm lg:text-[15px] leading-[14px] sm:leading-[16px] lg:leading-[18px] tracking-wide ${
              activeTab === "Singles"
      ...
      <div className="px-4 sm:px-5 pb-4 sm:pb-5 space-y-3 sm:space-y-4">
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            {/* Tournament Text */}
            <span className="text-xs sm:text-sm lg:text-[15px] font-normal leading-[14px] sm:leading-[16px] lg:leading-[18px] tracking-wide text-gray-900 truncate">
